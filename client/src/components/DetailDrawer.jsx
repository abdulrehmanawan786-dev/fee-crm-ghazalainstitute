...
export default function DetailDrawer({ student, onClose, onChanged, onEdit, onDeleteRequest }) {
  ...
  async function sendReminderForPayment(type) {
    ...
    try {
      await api.sendStudentReminder(student.id, type);
      const logs = await api.reminderLogs();
      setReminderLogs(logs.filter(l => l.student_id === student.id));
    } catch (e) { /* none */ }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(27,42,74,0.45)', display: 'flex', justifyContent: 'flex-end', zIndex: 40 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#F7F3EC', width: '100%', maxWidth: 520, minHeight: '100%', overflowY: 'auto', padding: 24, paddingBottom: 100, borderLeft: '2px solid #1B2A4A' }}>
